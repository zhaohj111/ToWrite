              ...
              {expanded && (
                ...
                >
                  ...
                  {creating?.type === "file" && creating.folderId === v.id && (
                    <li>{renderCreateInput("输入设定库文件名")}</li>
                  )}
                </ul>
              )}
            </li>
      ...
      <Dialog open={!!confirm} onOpenChange={(open) => !open && setConfirm(null)}>
        <DialogContent>
          {confirm?.kind === "file" && (
            <>
              <DialogHeader>
                <DialogTitle>删除设定库文件</DialogTitle>
                <DialogDescription>
                  确定删除「{confirm.title}」？该文件内的全部设定卡片与关系将一并删除，且无法撤销。
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
          ...
          {confirm?.kind === "folder" && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  「{confirm.title}」共 {byFolder.get(confirm.id)?.length ?? 0} 个设定库文件。请选择删除方式：
                </DialogDescription>
              </DialogHeader>
